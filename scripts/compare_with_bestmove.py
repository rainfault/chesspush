...
BESTMOVE_FEATURES = [
    ...
    "top2_move_ratio",
    "top3_move_ratio",
    "best_move_ratio_in_critical_positions",
    "best_move_ratio_under_time_pressure",
]
...
def main() -> None:
    ...
    bestmove_path = Path("data/engine/bestmove_features_120.csv")
    output_path = Path("data/models/baseline_cognitive_bestmove_comparison.json")
    ...
    if not bestmove_path.exists():
        raise FileNotFoundError(f"Bestmove dataset not found: {bestmove_path}")

    baseline_df = pd.read_csv(baseline_path)
    cognitive_df = pd.read_csv(cognitive_path)
    bestmove_df = pd.read_csv(bestmove_path)

    # Ориентируемся на subset из 120 партий
    ...
    baseline_plus_cognitive_plus_bestmove_features = baseline_plus_cognitive_features + [
        f for f in BESTMOVE_FEATURES if f in merged.columns
    ]

    experiments = {
        "baseline_only": baseline_only_features,
        "baseline_plus_cognitive": baseline_plus_cognitive_features,
        "baseline_plus_cognitive_plus_bestmove": baseline_plus_cognitive_plus_bestmove_features,
    }
