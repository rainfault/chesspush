...
# baseline БЕЗ Elo-признаков
BASELINE_WITHOUT_ELO_FEATURES = [
    "ply_count",
    "move_count_total",
    "avg_move_time",
    "median_move_time",
...
def main() -> None:
    ...
    output_path = Path("data/models/baseline_vs_cognitive_without_elo_300.json")
    ...
    baseline_features = [f for f in BASELINE_WITHOUT_ELO_FEATURES if f in merged.columns]
    baseline_plus_cognitive_features = baseline_features + [
        f for f in COGNITIVE_FEATURES if f in merged.columns
    ]

    experiments = {
        "baseline_without_elo": baseline_features,
        "baseline_without_elo_plus_cognitive": baseline_plus_cognitive_features,
    }
