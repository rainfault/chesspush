...
def main() -> None:
    ...
    cognitive_path = Path("data/engine/cognitive_features_300.csv")
    output_path = Path("data/models/baseline_vs_cognitive_results_300.json")
    ...
    # Берём только те baseline-строки, которые входят в cognitive subset
    ...
    if len(baseline_subset) != len(cognitive_df):
        print(
            f"Warning: baseline subset size ({len(baseline_subset)}) "
            f"!= cognitive size ({len(cognitive_df)})"
        )

    # Merge по game_id + label для надёжности
    ...
    baseline_only_df = merged.copy()
    baseline_plus_cognitive_df = merged.copy()

    baseline_only_features = [f for f in BASELINE_FEATURES if f in baseline_only_df.columns]
    baseline_plus_cognitive_features = baseline_only_features + [
        f for f in COGNITIVE_FEATURES if f in baseline_plus_cognitive_df.columns
    ]
    ...
    results = {
        ...
        "baseline_only_features": baseline_only_features,
        "baseline_plus_cognitive_features": baseline_plus_cognitive_features,
        "experiments": {
            "baseline_only": {
                "stratified_5fold_cv": run_stratified_cv(merged, baseline_only_features, n_splits=5),
                "leave_one_bot_out": run_leave_one_bot_out(merged, baseline_only_features),
            },
            "baseline_plus_cognitive": {
                "stratified_5fold_cv": run_stratified_cv(merged, baseline_plus_cognitive_features, n_splits=5),
                "leave_one_bot_out": run_leave_one_bot_out(merged, baseline_plus_cognitive_features),
            },
        },
    }
